...
#' Plot ChromStates
#'
#' @param x Dataframe with ChromStates OR data
...
#'
#' @export
plot_ChromStates <- function(x, outputdir = '.', outputfile = NULL, main='', xlab='',...)
{

   # x$RelIsland <- factor(x[,1], levels = (as.character(x[,1])))
   x$desc <- factor(x[,1], levels = (as.character(x[,1])))
   x[c("OR", "OR.inf","OR.sup","p-val")] <- lapply(x[c("OR", "OR.inf","OR.sup","p-val")], function(xf) {as.numeric(xf)} )
