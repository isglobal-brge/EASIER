...
#'
#' @export
summary_States_FP_Table <- function( significative, significativehyper, significativehypo, criteria, resformat = 'long', outputdir = '.', outputfile = NULL,  plot=TRUE )
{
   ...
   tmp <- data.frame( "States" = crit, "All_CpGs" = All_CpGs, "Sig_CpGs" = Sig_CpGs, "Sig_Hyper_CpGs" = Sig_Hypo_CpGs, "Sig_Hypo_CpGs" = Sig_Hyper_CpGs )

   if(resformat == 'long')
      tmp <- melt(tmp[,c("States","All_CpGs","Sig_CpGs", "Sig_Hyper_CpGs", "Sig_Hypo_CpGs")],id.vars = 1)
   ...
   if(plot){
      if(resformat != 'long'){
         dmp <- melt(tmp[,c("States","All_CpGs","Sig_CpGs", "Sig_Hyper_CpGs", "Sig_Hypo_CpGs")],id.vars = 1)
         plot_ProportionStatesFP( dmp, outputdir, outputfile)
      } else {
         plot_ProportionStatesFP( tmp, outputdir, outputfile)
      }
   }
