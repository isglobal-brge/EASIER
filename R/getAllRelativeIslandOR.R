...
#'
#' @export
getAllRelativeIslandOR <- function(significative, position, outputdir = ".", outputfile = NULL, plots = TRUE )
{

   positions <- unique(position)

   lregs <-  lapply(positions, function(x) getRelativeIslanOR(significative, ifelse(position == x, "yes", "no"), x) )
