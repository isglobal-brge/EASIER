...
if(!dir.exists(file.path(getwd(), paste0(folder,"/GWAMA/", names(metafiles)[metf],"_Filtr") )))
   suppressWarnings(dir.create(file.path(getwd(), 
                                         paste0(folder,"/GWAMA/", names(metafiles)[metf],"_Filtr"))))
...
modelfiles <- unlist(metafiles[metf])
   ...
   runs <- c('Normal', 'lowcpgs') # Execution with all CpGs and without filtered CpGs (low representation)
   list.lowCpGs <- NULL
   lowCpGs = FALSE;
   outputfiles <- list()
