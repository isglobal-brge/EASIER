      ...
      if(all == TRUE)
      {
         ...
         if(bn==TRUE)
         {
            ...
            if(!is.null(sigCpGs_bn) & length(sigCpGs_bn)>0)
            {
               ...
               Hs.c2.data <- gsameth(sig.cpg=sigCpGs_bn,  all.cpg=as.vector(data[,"CpGs"]), array.type = artype )
         ...
         if(!is.na(fdr)){
            ...
            if(!is.null(sigCpGs_fdr) & length(sigCpGs_fdr)>0) {
               ...
               # MSigDB enrichment - Only to work with a complete list of CpGs
               Hs.c2.data <- gsameth(sig.cpg=sigCpGs_fdr,  all.cpg=as.vector(data[,"CpGs"]), array.type = artype )
         ...
         if( !is.na(pval) ){
            ...
            if( !is.null(sigCpGs_pval) & length(sigCpGs_pval)>0 ) {
               ...
               # MSigDB enrichment - Only to work with a complete list of CpGs
               Hs.c2.data <- gsameth(sig.cpg=sigCpGs_pval,  all.cpg=as.vector(data[,"CpGs"]), array.type = artype )
