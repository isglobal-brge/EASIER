...
In ethnic variable we define the `ethnic` for each cohort.
...
```{r QCVarethnic}
# Exclude - MASK snp5
ethnic <- c('EUR', 'EUR', 'SAS', 'SAS', 'EUR', 'EUR', 'GMAF1p')
...
#### Perform CpG exclusions –> probes with hybridizing problems
  ...
  * **Poor mapping probes ("MASK_mapping")**: whether the probe is masked for mapping reason. Probes retained should have high quality (>=40 on 0-60 scale) consistent (with designed MAPINFO) mapping (for both in the case of type I) without INDELs. 
Probes that have poor quality mapping to the target genomic location as indicated in the array’s manifest file based on genome build GRCh38 (for example due to the presence of INDELs (Insertion–deletion mutations variants present in the genome); 
...
#### Perform CpG exclusions -> probes affected by the presence of SNPs
  ...
  * **"MASK_extBase"**: probes masked for extension base inconsistent with specified color channel (type-I) or CpG (type-II) based on mapping.
  Probes with a SNP altering the CpG dinucleotide sequence context and hence the ability of target cytosines to be methylated (regardless of the MAF); 
  * **"MASK_typeINextBaseSwitch"**: whether the probe has a SNP in the extension base that causes a color channel switch from the official annotation (described as color-channel-switching, or CCS SNP in the reference). These probes should be processed differently than designed (by summing up both color channels instead of just the annotated color channel). Probes with a SNP in the extension base that causes a colour channel switch from the official annotation (regardless of the MAF);
  * **"MASK_snp5_ethnic"**: whether 5bp 3’-subsequence (including extension for typeII) overlap with any of the common SNPs from dbSNP (MAF >1%) for a specific ethnic group which is defined in variable "MASK_snp5_ethnic". Valid ethnic groups can be seen in the next table  
  * **"MASK_snp5_GMAF1p"**: whether 5bp 3’-subsequence (including extension for typeII) overlap with any of the SNPs with global MAF >1\% across populations. Recommended if your population is very heterogeneous in terms of ethnicity.
  * **"MASK_snp5_common"**: whether 5bp 3’-subsequence (including extension for typeII) overlap with any of the common SNPs from dbSNP in any of the ethnic populations (global MAF can be under 1\%, but has to be >1% in at least one of the populations).
...
#### Perform CpG exclusions -> probes giving inconsistent results between arrays
  ...
  * **"Unrel_450_EPIC_pla_restrict"** or **"Unrel_450_EPIC_pla"**: These are probes that give inconsistent methylation levels between the 450K and EPIC arrays in placenta, suggesting that measurements are unreliable for at least one of these arrays. CpG probes based on [@fernandez2019comparison] . `Unrel_450_EPIC_pla_restrict` refers to CpG positions with extreme absolute differences in methylation levels when the two different platforms are compared and `Unrel_450_EPIC_pla` to probes showing low correlation between arrays and high variance
...
# {
   ...
   fResumeName <- paste0( file.path(getwd(), results_folder, prefixes[i]),
                          "/",prefixes[i], "_descriptives.txt")
...
We only take in to account ethnic group if `MASK_snp5_ethnic` is defined in `exclude` variable.
...
-	annotate CpGs with Illumina (hg19 (GRCh37)),
